...
class Credits extends Admin_Controller
{
    ...
    public function addStudent()
    {
        ...
        $data['section_credit_id']  = $this->input->post('section_credit_id');
        // $data['session_id']         = $this->input->get('session_id');
        if (!empty($data['students'])) {
            $this->credit_model->addStudent($data);
        }
        redirect('credits/view/'.$data['section_credit_id']);
    }
    ...
    public function addStudents()
    {
        ...
        redirect('credits/listStudents/'.$data['section_credit_id']);
    }
    ...
    public function listStudents($id)
    {
        ...
        $data['selected_class'] = $this->input->post('class');
        
        $this->session->set_userdata('top_menu', 'Student List');
        
        $data['title']          = 'Student List';
        $students               = $this->student_model->getStudentNotInCredit($id, $data);
        $data['students']       = $students;
        $data['credit']         = $this->credit_model->get($id);
        $data['class_list']     = $this->class_model->get();

        $this->load->view('layout/header', $data);
        $this->load->view('credit/listStudent', $data);
        $this->load->view('layout/footer', $data);
    ...
    public function view($id)
    {
        ...
        $data['title']   = 'Section Credit Students List';
        $credit          = $this->credit_model->get($id);
        $data['credit']  = $credit;
        
        $students         = $this->student_model->getStudentNotInCredit($id);
        $data['students'] = $students;
        
        $studentcredits         = $this->credit_model->getStudentsInCredit($id);
        $data['studentcredits'] = $studentcredits;

        $this->load->view('layout/header', $data);
        $this->load->view('credit/creditShow', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function deleteStudent($id, $idStudent)
    {   
        if (!$this->rbac->hasPrivilege('credits', 'can_delete')) {
            access_denied();
        }
        $data['title'] = 'Delete Student in Credit';
        $this->credit_model->removeStudent($id, $idStudent);
       
        redirect('credits/view/'.$id);
    }
