...
class Exam extends MY_Addon_CBSEController
{
    ...
    public function examstudent()
    {
        ...
        $examid = $_POST['examid'];
        $resultlist = $this->cbseexam_exam_model->searchExamStudents($examid);
        $data['exam_id'] = $examid;
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'so_tc' => $this->input->post('so_tc'),
                'so_gio' => $this->input->post('so_gio'),
                'lt' => $this->input->post('lt'),
                'th' => $this->input->post('th'),
                'bac_dao_tao_id' => $this->input->post('bac_dao_tao_id'),
    ...
    public function subjectstudent()
    {
        ...
        $resultlist = $this->cbseexam_exam_model->get_markexamstudents($this->input->post('exam_id'));
        $data['resultlist'] = $resultlist;
        $data['exam_assessment_types'] = $this->cbseexam_exam_model->get_exam_assessment_types($examdetails['cbse_exam_assessment_id']);
        $subject_detail = $this->batchsubject_model->getExamSubject($examdetails['subject_id']);
    ...
    public function show($id)
    {
        ...
        $examdetails = $this->cbseexam_exam_model->get_exambyId($data['exam_id']);
        $data['exam'] = $examdetails;
        $resultlist = $this->cbseexam_exam_model->get_markexamstudents($id);
        $data['resultlist'] = $resultlist;
        $data['exam_assessment_types'] = $this->cbseexam_exam_model->get_exam_assessment_types($examdetails['cbse_exam_assessment_id']);
        $subject_detail = $this->batchsubject_model->getExamSubject($examdetails['subject_id']);
        $data['subject_detail'] = $subject_detail;
        ...
        $this->load->view('layout/header');
        $this->load->view('cbseexam/exam/show', $data);
        $this->load->view('layout/footer');
    }
}
