...
class Student extends Admin_Controller
{
    ...
    public function download($enquiry_id, $doc_id)
    {
        $this->load->helper('download');
        $doc_details = $this->student_model->studentdocbyid($doc_id);
        $this->media_storage->filedownload($doc_details['doc'], "uploads/student_documents/" . $enquiry_id);
    }
    ...
    public function view($id)
    {
        ...
        $data['subjectgroup'] = $this->subjectgroup_model->getRowByID($student['subject_group_id']);
        $data['sch_setting'] = $this->sch_setting_detail;
        ...
        // subject group
        $resultlist_mark = $this->student_model->get_markexamstudents($id);
        $data['resultlist_mark'] = $resultlist_mark;
        // dd($resultlist_mark);
        // subject group
    ...
    public function deleteprofile($id)
    {
        ...
        $this->studentprofile_model->remove($id);
        $this->session->set_flashdata('msg', '<i class="fa fa-check-square-o" aria-hidden="true"></i> ' . $this->lang->line('delete_message') . '');
        redirect('student/createprofile');
    }

    public function doc_delete($id, $enquiry_id)
    {
    ...
    public function addStudent()
    {
        ...
        $students              = $this->input->post('items'); 
        foreach ($students as $value)
        {
            $student = $this->enquiry_model->getenquiry_list($value);
            if ($student) {
                
                $data_insert = array(
                'firstname'         => $student['name'],
                'rte'               => 'Mã sv',
                'mobileno'          => $student['contact'],
                'email'             => $student['email'],
                'admission_date'    => $this->customlib->dateFormatToYYYYMMDD(time()),
                ...
                $this->enquiry_model->enquiry_update($value, ['status' => 'enrolled']);
            }
            
            // add subject group
            $this->studentfeemaster_model->assign_bulk_fees($fee_session_group_id, $student_session_id, array());
            $subjectgroups = $this->subjectgroup_model->getRowByID($subject_group_id)->group_subject;
            foreach ($subjectgroups as $value)
            {
                $insert_array = array(
                    'subject_id' => $value->subject_id,
                    'student_id' => $insert_id,
                );
                $this->student_model->add_subject_student($insert_array);
            }
            // add subject group
        }
    }
    ...
    public function createprofile()
    {
        ...
        $data['title_list']            = 'Recently Added Student Profile';
        $data['studentprofilelist']  = $this->studentprofile_model->liststudentprofile();
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('student/studentCreateProfile', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $data = array(
                'firstname' => $this->input->post('firstname'),
            ...
            $this->studentprofile_model->add($data);
            $this->session->set_flashdata('msg', '<div class="alert alert-success">' . $this->lang->line('success_message') . '</div>');
            redirect('student/createprofile');
        }
    }
    ...
    public function editprofile($id)
    {
        ...
        $data['id']                    = $id;
        $student                       = $this->studentprofile_model->get($id);
        $genderList                    = $this->customlib->getGender();
        $data['student']               = $student;
        $data["month"]                 = $this->customlib->getMonthDropdown();
    ...
    public function studentlist()
    {
        ...
        $this->session->set_userdata('top_menu', 'Student List');
        ...
        $data['fields']          = $this->customfield_model->get_custom_fields('students', 1);
        $class                   = $this->class_model->get();
        $credit                  = $this->credit_model->get();
        $data['classlist']       = $class;

        $this->load->view('layout/header', $data);
        $this->load->view('student/studentListCredit', $data);
        $this->load->view('layout/footer', $data);
    ...
    public function dtstudentcreditlist()
    {
        $currency_symbol = $this->customlib->getSchoolCurrencyFormat();
        $class           = $this->input->post('class_id');
        $section         = $this->input->post('section_id');
    ...
    public function entrymarks()
    {
        ...
        $arr_mark = $this->input->post('mark');
        $student_id = $this->input->post('student_id');
        $mark_array = [];
        foreach ($this->input->post('subject_student_id') as $subject_key => $subject_id) {
            $insert_array = array(
                'subject_student_id' => $subject_id,
                'student_id' => $student_id,
                
            );
            foreach ($arr_mark[$subject_id] as $key => $value)
            {
                $insert_array[$key] = $value;
            }
            $mark_array[] = $insert_array;
        }
        $this->student_model->addresultmark_data($mark_array, $student_id, $subject_id);
        // $array = array('status' => '1', 'error' => '', 'message' => $this->lang->line('success_message'));
        // echo json_encode($array);
