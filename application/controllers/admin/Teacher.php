...
class Teacher extends Admin_Controller
{
    ...
    public function assign_class_credit_teacher()
    {
        if (!$this->rbac->hasPrivilege('assign_class_credit_teacher', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Academics');
        $this->session->set_userdata('sub_menu', 'admin/teacher/assign_class_credit_teacher');
        $data['title']      = 'Add Class Credit Teacher';
        $data['title_list'] = 'Class Credit List';

        $this->form_validation->set_rules(
            'subject', $this->lang->line('subject'), array(
                'required',
                // array('class_credit_exists', array($this->class_model, 'class_teacher_exists')),
            )
        );
        $this->form_validation->set_rules('section_credit', $this->lang->line('credits'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('teachers', $this->lang->line('class_teacher'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('supervisory_inspection', $this->lang->line('supervisory_inspection'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('academic_advisor', $this->lang->line('academic_advisor'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('technical_support', $this->lang->line('technical_support'), 'trim|required|xss_clean');

        if ($this->form_validation->run() == false) {

        } else {

            $class    = $this->input->post("section_credit");
            $subject  = $this->input->post("subject");
            $teachers = $this->input->post("teachers");
            $supervisory_inspection = $this->input->post("supervisory_inspection");
            $academic_advisor = $this->input->post("academic_advisor");
            ...
            $this->credit_model->update($section_array);
            ...
            $classteacherid = $this->input->post("classteacherid");
                ...
                if (isset($classteacherid)) {
                    ...
                    $data = array('id' => $classteacherid,
                        'section_credit_id'=> $class,
                        'subject_id'       => $subject,
                        'staff_id'         => $teachers,
                        'session_id'       => $this->current_session,
                    );
                } else {
                    $data = array('section_credit_id' => $class,
                        'subject_id'             => $subject,
                        'staff_id'               => $teachers,
                        'session_id'             => $this->current_session,
                    );
                }
            $this->classcreditteacher_model->addClassCreditTeacher($data);

            $this->session->set_flashdata('msg', '<div class="alert alert-success">' . $this->lang->line('success_message') . '</div>');
            redirect('admin/teacher/assign_class_credit_teacher');
        }
        $classlist         = $this->class_model->get();
        $data['classlist'] = $classlist;
        ...
        $assignteacherlist         = $this->credit_model->getSectionCreditTeacher();
        $data['assignteacherlist'] = $assignteacherlist;

        foreach ($assignteacherlist as $key => $value) {
            $subject_id         = $value["subject_id"];
            $section_credit_id  = $value["section_credit_id"];

            $tlist[] = $this->classcreditteacher_model->teacherByClassCreditSection($subject_id, $section_credit_id);
        }
        if (!empty($tlist)) {
            $data["tlist"] = $tlist;
        }
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('class/classCreditTeacher', $data);
        $this->load->view('layout/footer', $data);
    }
