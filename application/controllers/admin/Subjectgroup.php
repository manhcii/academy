...
class Subjectgroup extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules(
            'class_id', $this->lang->line('class'), array(
                'required',
                array('check_class_exists', array($this->subjectgroup_model, 'check_class_exists')),
            )
        );
    ...
    public function add()
    {
        ...
        $this->form_validation->set_rules('subject[]', $this->lang->line('subject'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('training_system', $this->lang->line('training_system'), 'required');
        $this->form_validation->set_rules(
            'class_id', $this->lang->line('class'), array(
                'required',
                array('check_class_exists', array($this->subjectgroup_model, 'check_class_exists')),
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->subjectgroup_model->add($class_array, $subject, $credit, $sections);
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
            redirect('admin/subjectgroup');
        }
        $subject_list             = $this->subject_model->get();
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/subjectgroup/subjectgroupAdd', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function edit($id)
    {
        ...
        $subject_list             = $this->subject_model->get();
        $data['subjectlist']      = $subject_list;
        $subjectgroupList         = $this->subjectgroup_model->getRowByID();
        $data['class_id']         = 0;
        $data['subjectgroupList'] = $subjectgroupList;
        $subjectgroup             = $this->subjectgroup_model->getRowByID($id);
        if (!empty($subjectgroup->sections)) {
            $data['class_id'] = $subjectgroup->class_id;
        }
        if (!empty($subjectgroup->group_subject)) {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $update_subjects = array_diff($subject,$delete_subjects);

            $this->subjectgroup_model->edit($class_array, $delete_subjects, $add_subjects,$update_subjects,$credit);
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
            redirect('admin/subjectgroup');
        }
    }
    ...
    public function getGroupByClass()
    {
        ...
        $data       = $this->subjectgroup_model->getsubjectByClass($class_id);
        echo json_encode($data);
    }
    ...
    {
        $data['subjectgroup'] = $this->subjectgroup_model->getRowByID($id);
        $this->load->view('layout/header', $data);
        $this->load->view('admin/subjectgroup/subjectgroupDetail', $data);
        $this->load->view('layout/footer', $data);
    }
