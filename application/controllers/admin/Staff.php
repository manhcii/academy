...
class Staff extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $data["role"]       = $staffRole;
        $data["role_id"]    = "";
        $data["teachercategory"] = $this->teachercategory_model->listcategory();
    ...
    public function profile($id)
    {
        ...
        if ($staff_info['positions']) {
            $positions = $this->position_model->get();
            $positionarr = [];
            foreach ($positions as $position) {
                if(in_array($position['id'], explode(",", $staff_info['positions']))) {
                    $positionarr[] = $position['name'];
                }
            }
            $staff_info['positions'] = implode(",", $positionarr);
        }
        
        if($staff_info['subjects']) {
            $subjects = $this->subject_model->get();
            $subjectarr = [];
            foreach ($subjects as $subject) {
                if(in_array($subject['id'], explode(",", $staff_info['subjects']))) {
                    $subjectarr[] = $subject['name'];
                }
            }
            $staff_info['subjects'] = implode(",", $subjectarr);
        }
        
        if ($staff_info['class']) {
            $class_name = $this->class_model->get($staff_info['class'])['class'];
        } else {
            $class_name = '';
        }
        ...
        $data['teachercategory']     = $this->teachercategory_model->get();
    ...
    public function create()
    {
        ...
        $roles                       = $this->role_model->get();
        $data["roles"]               = $roles;
        $data["positions"]           = $this->position_model->get();
        ...
        if ($this->form_validation->run() == true) {
            ...
            $category          = $this->input->post("type_of_teacher");
            $positions         = $this->input->post("positions");
            $subjects          = implode(",", $this->input->post("subjects"));
            $graduate_expertise = $this->input->post("graduate_expertise");
            ...
            if (isset($category)) {
                $data_insert['teacher_category'] = $category;
            }
            
            if (isset($graduate_expertise)) {
                $data_insert['graduate_expertise'] = $graduate_expertise;
            }
            
            if (isset($subjects)) {
                $data_insert['subjects'] = $subjects;
            }
            
            if (isset($positions)) {
                $data_insert['positions'] = implode(",", $positions);
            }
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data1 = array(
                ...
                'subjects'             => $subjects,
                'graduate_expertise'   => $graduate_expertise
            );
