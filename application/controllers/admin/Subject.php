...
class Subject extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('code', $this->lang->line('subject_code'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('course_credits', $this->lang->line('course_credits'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('training_system_id', $this->lang->line('training_system'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $courseCredits = $this->input->post('course_credits');
            $class = $this->input->post('class');
            $price = $this->class_model->get($class)['unit_price'];
            ...
            $data = array(
                ...
                'lecture_hours'     => $this->input->post('lecture_hours'),
                'lab_hours'         => $this->input->post('lab_hours'),
                'class'             => $class,
                'unit_price'        => $unit_price,
                // 'teachers'          => json_encode($this->input->post('teachers')),
            );
    ...
    public function edit($id)
    {
        ...
        $data['id']            = $id;
        $subject               = $this->subject_model->get_first($id);
        $data['subject']       = $subject;
        $data['subject_types'] = $this->customlib->subjectType();
        $data['trainingsystem']     = $this->training_system_model->get();
        $data['classes']            = $this->class_model->get();
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'type'              => $this->input->post('type'),
                'training_system_id'=> $this->input->post('training_system_id'),
                'course_credits'    => $this->input->post('course_credits'),
                'lecture_hours'     => $this->input->post('lecture_hours'),
                'lab_hours'         => $this->input->post('lab_hours'),
                'class'             => $this->input->post('class'),
                // 'teachers'          => json_encode($this->input->post('teachers')),
