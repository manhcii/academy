...
class Classsection_model extends MY_Model
{
    ...
    public function getClassByTrainingSystem($training_system_id) {
        ...
        if (isset($role_id) && ($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
            ...
            $this->db->where('classes.training_system_id', $training_system_id);
    ...
    public function getByID($id = null,$search_training_system = null)
    {
        $this->db->select('classes.*')->from('classes');
        if($search_training_system != null)
        {
            $this->db->where('training_system_id', $search_training_system);
        }
        else
        {
            $this->db->where('classes.training_system_id', 2);
        }
        if ($id != null) {
        ...
        if ($id != null) {
            ...
            if (!empty($vehicle_routes)) {
                foreach ($vehicle_routes as $vehicle_key => $vehicle_value) {
                    ...
                    $vec_route->unit_price          = $vehicle_value['unit_price'];
                    $vec_route->class_code          = $vehicle_value['class_code'];
    ...
    public function remove($section_id, $array)
    {
        $this->db->where('section_id', $section_id);
        $this->db->where_in('class_id', $array);
