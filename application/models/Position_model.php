...
class Position_model extends MY_Model {
    ...
    public function get($id = null) {
        ...
        if ($id != null) {
            $this->db->where('positions.id', $id);
        } else {
            $this->db->order_by('positions.id');
        }
    ...
    public function remove($id) {
        ...
        $message = DELETE_RECORD_CONSTANT . " On positions id " . $id;
        $action = "Delete";
        $record_id = $id;
    ...
    public function add($data) {
        ...
        if (isset($data['id'])) {
            ...
            $message = UPDATE_RECORD_CONSTANT . " On positions id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $message = INSERT_RECORD_CONSTANT . " On positions id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
            //======================Code End==============================
...
// # Updating data
    ...
    public function valid_position($str)
    {
        $name = $this->input->post('name');
        ...
        if ($this->check_position_exists($name, $id)) {
            $this->form_validation->set_message('check_exists', 'Record already exists');
            return false;
        } else {
            return true;
    ...
    public function check_position_exists($name, $id)
    {
        if ($id != 0) {
