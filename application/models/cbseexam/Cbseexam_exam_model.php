...
class Cbseexam_exam_model extends MY_Model {
    ...
    */
    ...
    {
       return $this->db->select('cbse_exams.*,classes.class AS classse_name,cbse_terms.name as term_name, training_system.name as bac_dao_tao, subjects.name as subject_name')->from('cbse_exams')
       ->join('classes','classes.id=cbse_exams.class_id')
       ->join('cbse_terms','cbse_terms.id=cbse_exams.cbse_term_id','left')
       ->join('subjects','subjects.id=cbse_exams.subject_id','left')
       ->join('training_system','training_system.id=cbse_exams.bac_dao_tao_id','left')->group_by('cbse_exams.id')->order_by('cbse_exams.id','desc')->where('session_id', $this->current_session)->get()->result_array();        
    }
    ...
    */
    ...
    public function add_exam_class_section($data)
    {
     ...
     public function searchExamStudents($exam_id) {
    ...
    public function getStudentExamByStudentSession($student_session_id){
        return $this->db->select('cbse_exam_students.*,cbse_exams.cbse_exam_assessment_id,cbse_exams.cbse_term_id,cbse_exams.name,cbse_exams.use_exam_roll_no,cbse_exams.is_active,cbse_exams.is_publish,cbse_exams.cbse_term_id,cbse_exams.total_working_days')
    ...
    public function get_markexamstudents($exam_id){
        $result=array();
        $student_data= $this->db->select('students.*,cbse_exam_students.id as exam_student_id,cbse_exam_students.roll_no as `exam_roll_no`,classes.class as class_name,sections.section as section_name')
        ->from('cbse_exam_students')
        ->join('student_session','student_session.id=cbse_exam_students.student_session_id')
        ->join('classes', 'student_session.class_id = classes.id')
        ->join('sections', 'sections.id = student_session.section_id') 
        ...
        foreach($student_data as $key=>$value){
            $cbse_student_subject_marks=$this->db->select('cbse_student_subject_marks.*')
                        ...
                        ->where(array(' `cbse_student_subject_marks`.`cbse_exam_student_id` '=>$value['exam_student_id']))
                        ->get()
                        ->result_array();
