...
class Student_model extends MY_Model
{
    ...
    {
        $this->db->select('pickup_point.name as pickup_point_name,student_session.route_pickup_point_id,student_session.transport_fees,students.app_key,students.parent_app_key,student_session.vehroute_id,vehicle_routes.route_id,vehicle_routes.vehicle_id,transport_route.route_title,vehicles.vehicle_no,hostel_rooms.room_no,vehicles.driver_name,vehicles.driver_contact,vehicles.vehicle_model,vehicles.manufacture_year,vehicles.driver_licence,vehicles.vehicle_photo,hostel.id as `hostel_id`,hostel.hostel_name,room_types.id as `room_type_id`,room_types.room_type ,students.hostel_room_id,student_session.id as `student_session_id`,student_session.fees_discount,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,classes.unit_price,students.id,students.admission_no,students.roll_no,students.admission_no,students.admission_date,students.firstname,students.middlename, students.lastname,students.image,students.mobileno, students.email ,students.state,students.city,students.pincode,students.note,students.religion,students.cast, school_houses.house_name,students.dob,students.current_address,students.previous_school,students.guardian_is,students.parent_id,  students.permanent_address,students.category_id,students.adhar_no,students.samagra_id,students.subject_group_id,students.bank_account_no,students.bank_name, students.ifsc_code,students.guardian_name,students.father_pic ,students.height,students.weight,students.measurement_date, students.mother_pic,students.guardian_pic,students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.father_phone,students.blood_group,students.school_house_id,students.father_occupation,students.mother_name,students.mother_phone,students.mother_occupation,students.guardian_occupation,students.gender,students.guardian_is,students.rte,students.guardian_email, users.username,users.password,users.id as user_id,students.dis_reason,students.dis_note,students.disable_at,classesSubject.class as classesSubject')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('hostel_rooms', 'hostel_rooms.id = students.hostel_room_id', 'left');
        ...
        $this->db->join('school_houses', 'school_houses.id = students.school_house_id', 'left');
        $this->db->join('users', 'users.user_id = students.id', 'left');
        $this->db->join('subject_groups', "subject_groups.id = students.subject_group_id", 'left');
        $this->db->join('classes as classesSubject', "classesSubject.id = subject_groups.class_id", 'left');
        $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('users.role', 'student');
        if ($id != null) {
            $this->db->where('students.id', $id);
    ...
    public function getStudentNotInCredit($id, $data = [])
    {
        $this->db->select('students.*, classes.class')->from('students');
        $this->db->join('subject_students', 'subject_students.student_id = students.id');
        $this->db->join('section_credit_subjects', 'section_credit_subjects.subject_id = subject_students.subject_id');
        $this->db->join('section_credits', 'section_credit_subjects.section_credit_id = section_credits.id');
        ...
        if (!empty($data) && !empty($data['selected_class'])) {
            $this->db->where('classes.id', $data['selected_class']);
        }
        $this->db->where('section_credits.id', $id);
        // Khác trạng thái đang học,đã qua môn
        $this->db->where_not_in('subject_students.status', [1, 2]);
        $query = $this->db->get();
        return $query->result_array();
        
        // $students = $this->db->select('student_id')->from('section_credits_students')->where('section_credit_id', $id)->get()->result_array();
        // $students = array_column($students, 'student_id');
        // $this->db->select()->from('students');
        // if (!empty($students)) {
        //     $this->db->where_not_in('students.id', $students);
        // }
        // $query = $this->db->get();
        // return $query->result_array();
    }
    ...
    public function getenquirydoc($id)
    {
        $this->db->select()->from('student_doc');
    ...
    public function add_subject_student($insert_array) {
        $delete_array = array();
        $new_inserted_array = array('0');
        $this->db->trans_begin();
        if (!empty($insert_array)) {
            $this->db->insert('subject_students', $insert_array);
        }
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
            return false;
    ...
    public function get_markexamstudents($id){
        $result=array();
        $student_data= $this->db->select('subject_students.*,subjects.name as subjects_name,students.admission_no as admission_no,subjects.code as subjects_code')
        ->from('subject_students')
        ->join('students','students.id=subject_students.student_id')  
        ->join('subjects','subjects.id=subject_students.subject_id') 
        ->where('subject_students.student_id',$id)
        ->get()->result_array();

        foreach($student_data as $key=>$value){
            $student_subject_marks=$this->db->select('student_subject_marks.*')
                        ->from('student_subject_marks')
                        ->where(array('student_subject_marks.subject_student_id'=>$value['id'],' `student_subject_marks`.`student_id` '=>$value['student_id']))
                        ->get()
                        ->row_array();
    ...
    public function addresultmark_data($result_mark,$student_id,$subject_id){
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        ...
        if (isset($result_mark['id']) && !empty($result_mark['id'])) {
            ...
            $this->db->update('student_subject_marks', $result_mark);            
            ...
            foreach ($result_mark as $mark_key => $mark_value) {
               $this->db->insert('student_subject_marks', $mark_value);
               $insert_id = $this->db->insert_id();
               $insert_ids[]=$insert_id;
            }
            if (!empty($insert_ids)) {
                $this->db->where('subject_student_id', $subject_id);
                $this->db->where('student_id', $student_id);
                $this->db->where_not_in('id', $insert_ids);
                $this->db->delete('student_subject_marks');
            }
        }
