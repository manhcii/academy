...
class Subjectgroup_model extends MY_Model {
    ...
    public function edit($data, $delete_subjects, $add_subjects,$update_subjects,$credit_group) {
        $this->db->trans_begin();
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
        ...
        if (!empty($update_subjects)) {
            $subject_group_subject_Array = array();
            foreach ($update_subjects as $sub_group_key => $sub_group_value) {
                $vehicle_array = [];
                $vehicle_array = array(
                    'credit'     => $credit_group[$sub_group_key],
                );

                $this->db->where('subject_group_id', $data['id']);
                $this->db->where('subject_id', $sub_group_value);
                $this->db->update('subject_group_subjects', $vehicle_array);
            }
        }
    ...
    public function add($data, $subject_group, $credit_group, $section_group) {
    ...
    public function getByID($id = null) {
        $this->db->select('subject_groups.*,classes.class as class,classes.class_code as class_code')->from('subject_groups');
        $this->db->join('classes', 'subject_groups.class_id = classes.id');
        ...
        if (!empty($subject_groups)) {
           foreach ($subject_groups as $subject_group_key => $subject_group_value) {
                ...
                $subject_groups[$subject_group_key]->sections = $this->getRowClassSectionByGroup($subject_group_value->id);
            }
        }
    ...
    public function getRowByID($id = null) {
        ...
        $query = $this->db->get();
        $subject_groups = $query->row();
        if (!empty($subject_groups)) {
            $subject_groups->group_subject = $this->getGroupsubjects($subject_groups->id);
            $subject_groups->sections = $this->getRowClassSectionByGroup($subject_groups->id);
        }
        return $subject_groups;
    }
    ...
    public function getRowClassSectionByGroup($subject_group_id) {

        $sql = "SELECT subject_group_class_sections.*,classes.id as `class_id`,classes.class,sections.id as `section_id`,sections.section FROM `subject_group_class_sections` INNER JOIN class_sections on class_sections.class_id=subject_group_class_sections.class_id INNER JOIN classes on classes.id=class_sections.class_id INNER join sections on sections.id=class_sections.section_id WHERE subject_group_class_sections.session_id=" . $this->db->escape($this->current_session) . " and subject_group_id=" . $this->db->escape($subject_group_id);
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    public function check_class_exists($str) {
        ...
        if ($this->check_class_data_exists($class_id, $id)) {
    ...
    function check_class_data_exists($class_id, $id) {
