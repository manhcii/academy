...
class Studentprofile_model extends MY_Model
{
    ...
    public function liststudentprofile()
    {
        $this->db->select()->from('student_profile');
        $this->db->where('status', '1');
        $this->db->order_by("id", "desc");
        $listprofile = $this->db->get();
        return $listprofile->result_array();
    }
    ...
    public function remove($id)
    {
        ...
        $message   = DELETE_RECORD_CONSTANT . " On student profile id " . $id;
        $action    = "Delete";
        $record_id = $id;
    ...
    public function add($data, $data_setting = array())
    {
        if (isset($data['id'])) {
            ...
            $message   = UPDATE_RECORD_CONSTANT . " On student_profile id " . $data['id'];
            $action    = "Update";
